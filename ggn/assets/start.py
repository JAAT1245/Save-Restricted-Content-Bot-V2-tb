...
async def start(event):
    ...
    collection.update_one({"user_id": user_id}, {"$set": {"user_id": user_id}}, upsert=True)
    buttons = [
        [Button.url("Join Channel", url="https://t.me/TARGETALLCOURSE")],
        [Button.url("Contact Me", url="https://t.me/Free_course2_bot")],
    ]
...
PRE_TEXT = """💰 **Premium Price**: Starting from $3 or 300 INR accepted via **__Amazon Gift Card__** (terms and conditions apply).
...
async def plan_command(event):
    ...
    buttons = [
        [Button.url("Send Gift Card Code", url="https://t.me/Free_course2_bot")]
    ]
...
async def term_command(event):
    # Creating inline keyboard with buttons
    buttons = [
        [Button.url("Query?", url="https://t.me/Free_course2_bot"),
         Button.url("Channel", url="https://telegram.dog/TARGETALLCOURSE")]
    ]
...
async def youtube_dl_command(_, message):
    ...
    if len(message.command) > 1:
        ...
        try:
            ...
            caption = f"{video_info['title']}\n\n__**Powered by [Advance Content Saver Bot](https://t.me/Free_course2_bot)**__"  # Set caption to the title of the video
